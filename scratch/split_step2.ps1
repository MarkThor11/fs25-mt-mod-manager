...
if (!(Test-Path $safe)) { New-Item -ItemType Directory -Path $safe }
...
$count = [Math]::Floor($newSuspects.Count / 2)
Write-Host "Splitting suspect group. Moving $count mods back to $onHold..."
for ($i=0; $i -lt $count; $i++) {
    Move-Item $newSuspects[$i].FullName $onHold
}

Write-Host "Done. Ready for next test."
